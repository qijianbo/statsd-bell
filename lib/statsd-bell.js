...
function Bell(uptime, config, events) {
  ...
  var connect = function() {
    // connect to bell
    bell.conn = net.connect({host: bell.host, port: bell.port}, function(){
      if (debug) logger.log('bell connected successfully.');
    });

    // reconnect on error
    bell.conn.on('error', function(err) {
      if (debug) logger.log('bell ' + err.message + ', closing it..');
      bell.conn.destroy();
      if (debug) logger.log('try to reconnect to bell in 10s..');
